...
choose_best = [0 1]; %0이면 air, 1이면 테스트로 결정한 변수로 재제작
...
%%%%%%%%%%%%%%%%%%%%%%%%   도화지 설정   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for best = choose_best
    if best == 1
        % 테스트로 확인한 최대효율의 변수로 설정
        depth=200;  %화면방향 깊이
        slot_number= 3; % 슬롯 개수 설정
        r_so=300;   % 스테이터 직경 100
        r_si=180; % r_so - r_rotor_outer > r_si
        ...
        c_w=30; %코일의 너비
        c_h=30; %코일의 높이
        l_c=255; %중심점(0,0)으로 부터의 코일 위치

        % 로터 변수
        r_rotor_outer=165; %로터 바깥쪽 반지름

        % 자석 변수
        num_pole=4;       %회전자 자석 개수
        m_w=90;           %자석의 너비
        m_th=30;           %자석의 높이
        l_m=120;           %중심점(0,0)으로부터의 자석 위치
...
xlabel('Is-BestTest [Boolean]');

yyaxis left;
plot(choose_best,T_average_result,'b-','LineWidth',2); % 전류밀도에 따른 토크 그래프
ylabel('Torque[Nm]');

yyaxis right;
plot(choose_best,T_ripple_result,'r--','LineWidth',2); % 전류밀도에 따른 리플 토크 그래프
ylabel('Ripple Torque[%]');

grid on;
axis auto;
title('Torque by Best Test');