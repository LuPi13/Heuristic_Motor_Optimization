...
for kk=1:N
    ...
    Fx(kk) = mo_blockintegral(18); %로터 x방향 힘 불러오기
    Fy(kk) = mo_blockintegral(19); %로터 y방향 힘 불러오기
...
mi_addsegment(x1,y1,x1,y2);	mi_addsegment(x1,y2,x2,y2);	mi_addsegment(x2,y2,x2,y1);	mi_addsegment(x2,y1,x1,y1);
end

function drawcircle(x1,y1,r) %(x1, y1) 점을 기준으로 r 반지름의 원 크리기 함수
mi_addnode(x1+r,y1);
mi_addnode(x1-r,y1);
